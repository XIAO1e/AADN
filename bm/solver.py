...
from .losses import ClipLoss, FeatureDecodingLoss, L1Loss, L2Loss, AADNLoss
...
class Solver(flashy.BaseSolver):
    ...
    def _create_loss(self, loss: str):
        if loss == 'l1':
            ...
            clip_kw = dict(self.args.clip)
            aadn_kw = dict(self.args.aadn)
            loss = AADNLoss(clip_args=clip_kw, aadn_args=aadn_kw, dset_args=self.args.dset)
            if self.optimizer is not None:
                self.optimizer.add_param_group({"params": loss.parameters()})
            return loss
        else:
